...
class BasicConv(nn.Module):
    def __init__(self, in_planes, out_planes, kernel_size,
        ...
        self.conv = nn.Conv2d(in_planes, out_planes, kernel_size=(kernel_size, kernel_size),
                              stride=(stride, stride), padding=(padding, padding),
                              dilation=(dilation, dilation), groups=groups, bias=bias)
        self.bn = nn.BatchNorm2d(out_planes, eps=1e-5, momentum=0.01, affine=True) if batch_norm else None
...
class CBAM(nn.Module):
    def __init__(self, gate_channels, reduction_ratio=16, pool_types=('avg', 'max'), no_spatial=False):
        ...
        self.ChannelGate.apply(init_weights)
        if not no_spatial:
            self.SpatialGate = SpatialGate()
            self.SpatialGate.apply(init_weights)
